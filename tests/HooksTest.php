...
class HooksTest extends \PHPUnit\Framework\TestCase
{
	...
	public function testRunHookFunctions ()
	{
		...
		var_export($hooks->do_action('testAction'));
		...
		self::assertSame(false, $hooks->do_action_ref_array('testAction', [
			'test'
		]));
		self::assertSame('Foo', $hooks->apply_filters_ref_array('testFilter', [
			'Foo'
		]));
		...
		self::assertSame(true, $hooks->add_action('testAction', [
			$mock,
			'doSomeAction'
		]));
		self::assertSame(true, $hooks->add_filter('testFilter', [
			$mock,
			'applySomeFilter'
		]));
		...
		self::assertSame('foo', $hooks->apply_filters('testFilter', 'Foo'));
		self::assertSame(true, $hooks->add_filter('all', [
			$mock,
			'applySomeFilter'
		]));
		...
		self::assertSame(true, $hooks->do_action('testAction', (object)[
			'foo' => 'bar'
		]));
		self::assertSame(true, $hooks->do_action('testAction', 'param1', 'param2', 'param3', 'param4'));
		self::assertSame(true, $hooks->do_action_ref_array('testAction', [
			'test'
		]));
		self::assertSame('foo', $hooks->apply_filters('testFilter', 'Foo'));
		self::assertSame('foo', $hooks->apply_filters_ref_array('testFilter', [
